...
import { Clock, Search, Undo2, Loader2, Check } from 'lucide-react';
...
interface PreRegProps {
 ...
 totalCredits: number;
 course_capacities?: Record<string, number>; // Handle potential prop name mismatch
...
export function PreReg({
 ...
 courseCapacities
  ...
  const onAddClick = async (course: Course) => {
    setAddingCourseId(course.id);
    // Simulate API delay as per Finding 3 (2-4 seconds)
    await new Promise(resolve => setTimeout(resolve, 2000));
    setAddingCourseId(null);
    setSuccessCourseId(course.id);
    handleAddCourse(course);
    // Show checkmark for 1.5s as per Finding 3
    setTimeout(() => setSuccessCourseId(null), 1500);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <table className="mfu-table text-sm">
                ...
                <tbody>
                  {filteredCourses.map(course => {
                    const currentCapacity = courseCapacities[course.id] || 40;
                    const enrolledCount = course.capacity - course.remainingSeats;
                    const currentRemaining = Math.max(0, currentCapacity - enrolledCount);
                    
                    return (
                      <tr key={course.id}>
                        <td className="font-bold text-mfu-red text-sm">{course.code}</td>
                        <td className="text-sm">{course.name}</td>
                        <td className="text-[11px]">{course.faculty}</td>
                        <td className="text-[11px]">{course.day} {course.time}</td>
                        ...
                        <td>
                          <button
                            ...
                            disabled={isProcessing || addingCourseId === course.id || (currentRemaining <= 0 && !registeredCourses.includes(course.id) && !submittedCourses.includes(course.id))}
                            className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all flex items-center justify-center gap-1.5 min-w-[70px] ${
        ...
        <div className="space-y-4">
          <div className="mfu-card p-4 space-y-4 sticky top-24">
            <h3 className="text-lg font-bold text-mfu-text-main border-b border-mfu-border pb-2">Registration Plan</h3>
